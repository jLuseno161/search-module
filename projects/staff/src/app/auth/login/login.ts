...
export class Login implements OnInit {
  ...
  private navigateBasedOnRole(userRole: string): void {
    ...
    if (!userRole) {
      console.error('No user role provided for navigation');
      this.router.navigate(['/login']);
      return;
    }
    ...
    switch (userRole) {
      ...
      case 'superuser':
        ...
        break;
      case 'is_registrar_in_charge':
        targetPath = '/registrarInCharge';
        break;
      case 'is_registrar':
        targetPath = '/registrar-dashboard';
        break;
      default:
    ...
    // Try navigation and handle result
    this.router.navigate([targetPath]).then(success => {
      console.log(`Navigation to ${targetPath}:`, success);
      if (!success) {
        console.error(`Failed to navigate to ${targetPath}`);
        // Try alternative paths
        if (targetPath === '/registrar-dashboard') {
          console.log('Trying alternative path: /registrar');
          this.router.navigate(['/registrar']).then(altSuccess => {
            console.log('Alternative navigation success:', altSuccess);
            if (!altSuccess) {
              this.router.navigate(['/login']);
            }
          });
        } else if (targetPath === '/registrarInCharge') {
          console.log('Trying alternative path: /chief-registrar');
          this.router.navigate(['/chief-registrar']).then(altSuccess => {
            console.log('Alternative navigation success:', altSuccess);
            if (!altSuccess) {
              this.router.navigate(['/login']);
            }
