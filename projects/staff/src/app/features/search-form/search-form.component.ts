...
export class SearchComponent implements OnInit, AfterViewInit {
  ...
  @Output() nextToUpload = new EventEmitter<any>(); // New event for next button
  ...
  isSaved: boolean = false; // Track if certificate has been saved
  generatedCertificateData: any = null; // Store generated certificate data
  ...
  ngOnInit() {
    ...
    this.isSaved = false; // Reset saved state
  ...
  /**
   * Save Certificate - Generate PDF and save
   */
 // In search.component.ts
// In search-form.component.ts
...
async onSaveCertificate() {
  if (this.searchForm.valid) {
    ...
    try {
      ...
      // ✅ Store ONLY metadata, not the full PDF
      const metadata = {
        applicationId: this.applicationId,
        referenceNumber: this.referenceNumber,
      ...
      console.log('✅ Certificate metadata saved to localStorage:', {
      ...
      // Emit to parent with metadata only
...
// goToUploadTab() {
//   if (this.isSaved && this.generatedCertificateData) {
//     // Pass only the necessary data, not the blob
...
goToUploadTab() {
  if (this.isSaved && this.generatedCertificateData && this.generatedCertificateData.blob) {
    ...
    // Pass the actual file and blob
    this.nextToUpload.emit({
      certificateFile: certificateFile,  // Pass the file object
      certificateBlob: this.generatedCertificateData.blob,  // Also pass blob
      applicationId: this.applicationId,
      referenceNumber: this.referenceNumber,
      certificateData: this.generatedCertificateData.data
    });
...
//         // Store in localStorage with application ID
...
// goToUploadTab() {
//   if (this.isSaved && this.generatedCertificateData) {
//     // Pass the storage key to parent
//     this.nextToUpload.emit({
//       storageKey: `certificate_${this.applicationId}`,
//       applicationId: this.applicationId,
//       referenceNumber: this.referenceNumber,
//       certificateData: this.generatedCertificateData.data
//     });
...
//   }
  ...
  /**
   * Go to Certificate Upload tab
   */
  // goToUploadTab() {
  //   if (this.isSaved && this.generatedCertificateData) {
  //     this.nextToUpload.emit(this.generatedCertificateData);
  //   } else {
  //     this.snackBar.open('Please save the certificate first before proceeding', 'Close', { duration: 3000 });
  //   }
  // }
